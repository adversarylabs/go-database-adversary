...
export const GO_DATABASE_MODEL_SCHEMA: Record<string, unknown> = {
  type: "object",
...
export interface ModelDatabaseObservation {
  id: string;
  title: string;
  category: string;
...
export interface ModelDatabaseReview {
  assessment: {
  ...
  observations: ModelDatabaseObservation[];
}
...
export async function applyModelDatabaseReview(
  ctx: RuleContext,
  output: ModelDatabaseReview,
  evidenceById: Map<string, PreparedEvidenceItem>,
...
export async function runModelDatabaseReview(
  ctx: RuleContext,
  analysis: Analysis,
  files: DiscoveryFile[],
  staticSeverities: StaticSeverity[] = [],
  ...
  try {
    const result = await ctx.model.review<ModelDatabaseReview>(request);
...
function severityRank(severity: StaticSeverity | ModelDatabaseObservation["severity"]): number {
  switch (severity) {
    case "critical":
      return 4;
    case "high":
...
function maxSeverity(values: Array<StaticSeverity | ModelDatabaseObservation["severity"]>): StaticSeverity {
